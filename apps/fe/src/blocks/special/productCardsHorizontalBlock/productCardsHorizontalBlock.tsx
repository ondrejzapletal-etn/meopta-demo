...
import { ProductCardsHorizontalBlock as ProductCardsHorizontalBlockType } from '@repo/shared/payload-types';
...
export const ProductCardsHorizontalBlock = ({
  title,
  items,
}: ProductCardsHorizontalBlockType) => {
    ...
    <div className="w-full bg-white">
      ...
      >
        ...
        <div ref={ref} className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8">
          {items.map((item, i) => {
            const imgSrc = getImageSrcWithFallback(item.image, { width: 400, height: 300 });
            ...
            return (
              ...
              >
                {imgSrc && (
                  <div className="relative aspect-[4/3] w-full flex-shrink-0 overflow-hidden md:aspect-auto md:w-[200px] lg:w-[240px]">
                    <Image
                      src={imgSrc}
                      alt={item.title ?? ''}
                      fill
