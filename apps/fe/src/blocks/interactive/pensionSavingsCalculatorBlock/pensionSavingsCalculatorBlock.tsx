...
import { PensionSavingsCalculatorBlock as PensionSavingsCalculatorBlockType } from '@repo/shared/payload-types';
...
export const PensionSavingsCalculatorBlock = ({
  title,
  ...
  stateContribution: stateContributionProp,
  description,
}: PensionSavingsCalculatorBlockType) => {
  const minMonthly = minMonthlyProp ?? 300;
  const maxMonthly = maxMonthlyProp ?? 10000;
  const defaultMonthly = defaultMonthlyProp ?? 1000;
  const minYears = minYearsProp ?? 5;
  const maxYears = maxYearsProp ?? 40;
  const defaultYears = defaultYearsProp ?? 20;
  const defaultReturn = expectedReturnProp ?? 4;
  const stateContribution = stateContributionProp ?? 230;
  ...
  const results = useMemo(() => {
    const totalMonthly = monthly + stateContribution;
    const r = returnRate / 100 / 12;
    const n = years * 12;
    const futureValue = totalMonthly * ((Math.pow(1 + r, n) - 1) / r);
    const totalDeposited = monthly * n;
    const totalState = stateContribution * n;
    const profit = futureValue - totalDeposited - totalState;
    return { futureValue, totalDeposited, totalState, profit };
  }, [monthly, years, returnRate, stateContribution]);
      ...
      >
        ...
        >
          <div className="flex flex-col gap-8">
            ...
            <Slider label="Ocekavany rocni vynos" min={1} max={10} step={0.5} value={returnRate} onChange={setReturnRate} formatValue={formatPercent} />
          </div>
          ...
          <div className="rounded-2xl bg-meopta-blue-light p-6 lg:p-8">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between border-b border-meopta-border pb-3">
                <span className="text-14 text-meopta-text-secondary">Vase ukladky celkem</span>
                <span className="text-16 font-semibold text-meopta-text-primary">{formatCurrency(Math.round(results.totalDeposited))}</span>
              </div>
              <div className="flex items-center justify-between border-b border-meopta-border pb-3">
                <span className="text-14 text-meopta-text-secondary">Statni prispevek celkem</span>
                <span className="text-16 font-semibold text-meopta-blue-dark">{formatCurrency(Math.round(results.totalState))}</span>
              </div>
              <div className="flex items-center justify-between border-b border-meopta-border pb-3">
